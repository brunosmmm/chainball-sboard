...
class WebBoard(object):
    ...
    def register(self):
        ...
        try:
            ...
            self.logger.debug(
                "Could not register player {}: invalid text".format(player)
            )
    ...
    def set_score(self, player, score):
        """Set score (force)."""
        try:
            self.game.game_force_score(int(player), int(score))
        except KeyError:
            return {"status": "error", "error": "Invalid player"}
        except TypeError:
            return {"status": "error", "error": "Malformed request"}
    ...
    def dump_game_readable(self, game_uuid):
        ...
        try:
            ...
            self.logger.error(
                "Caught exception in dump_game_readable: {}".format(repr(ex))
            )
            return Response(
                "An error occured while processing this request", status=500
            )
    ...
    def dump_game_range(self, start_uuid, count):
        ...
        if (
            uuid_from_int(int(start_uuid) + int(count) - 1)
            not in self.game.g_persist.game_history
        ):
            response.status = 500
        ...
        for uuid in range(int(start_uuid), int(start_uuid) + int(count)):
            ...
            game_dump.append(
                ["EVENT_TYPE", "EVENT_TIMESTAMP", "EVENT_PLAYER", "EVENT_INFO"]
            )
            game_end_rtime = None
    ...
    def generate_event_info_field(self, event):
        ...
        if event_type in ("SCORE_FORCED", "SCORE_CHANGE"):
            ...
            ret = "OLD = {}; NEW = {} ({:+d})".format(
                event_desc["old_score"],
    ...
    def run(self):
        ...
        # better server
        run(host=bind_to, port=self.port, server="cheroot")
